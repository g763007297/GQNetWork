...
typedef NSInputStream * (^GQRequestNeedNewBodyStream)(GQBaseDataRequest * request,NSURLSession *session,NSURLSessionTask *task);//需要新的数据流block
...
- (NSInputStream *)requestNeedNewBodyStream:(GQBaseDataRequest *)request session:(NSURLSession *)session task:(NSURLSessionTask *)task;//需要新的数据流代理
